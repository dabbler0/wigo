...
# # Chased
exports.Chased = class Chased extends Game
  constructor: (@w = 5, @h = 5) ->
    super @w * @h, 4, 3

    # Place player
    @state.layers[0][0] = 1
    @state.layers[1][@w * @h - 1] = 1
  ...
  advance: (action) ->
    ...
    # Get the current chaser coordinate
    chaserCoord = null
    for i in [0...@w * @h]
      if @state.layers[1][i] is 1
        chaserCoord = @_coord i
        break
    ...
    chaser = (@state.layers[1][@_index(newCoord)] is 1)

    # Move the prize
    oldIndex = @_index chaserCoord
    if chaser
      chaserCoord.x = @w - 1
      chaserCoord.y = @h - 1
    else
        if chaserCoord.x > newCoord.x 
          dir = _dirs[3]
        if chaserCoord.x < newCoord.x
          dir = _dirs[1]
        if chaserCoord.x = newCoord.x
          if chaserCoord.y > newCoord.y
            dir = _dirs[2]
          if chaserCoord.y > newCoord.y
            dir = _dirs[0]
      chaserCoord.x += dir.x
